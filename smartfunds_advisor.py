...
PICKS = [
    ...
    {"cat":"fd","name":"PPF Account","ticker":"PPF (15yr)","why":"EEE tax status. 7.1% risk-free — best debt instrument in India.","ret":"7.1% tax-free","risk":"None","sector":"Debt"},
    ...
    {"cat":"ppf","name":"National Pension System","ticker":"NPS Tier-1","why":"Extra ₹50K tax deduction under 80CCD(1B). Market-linked returns.","ret":"~10–12% CAGR","risk":"Low","sector":"Pension"},
...
def months_to_goal(monthly_sip, rate_annual, target):
    ...
    # Solve: monthly * (((1+r)^n - 1)/r)*(1+r) = target
    # Binary search
    lo, hi = 1, 600
    while lo < hi:
        mid = (lo + hi) // 2
...
def tax_old_regime(income):
    """Income tax under old regime (FY 2024-25)"""
    slabs = [(250000, 0), (500000, 0.05), (1000000, 0.20), (float('inf'), 0.30)]
    deductions = 150000  # 80C
    taxable = max(0, income - deductions - 50000)  # std deduction
    tax = 0
    prev = 0
...
def tax_new_regime(income):
    """Income tax under new regime (FY 2024-25) - default"""
    slabs = [(300000, 0), (600000, 0.05), (900000, 0.10), (1200000, 0.15), (1500000, 0.20), (float('inf'), 0.30)]
    taxable = max(0, income - 75000)  # std deduction new regime
    tax = 0
    prev = 0
    for limit, rate in slabs:
        if taxable <= prev: break
...
<div style="padding: 2rem 0 1.5rem; border-bottom: 1px solid #1e2235; margin-bottom: 2rem; display: flex; align-items: center; justify-content: space-between;">
  <div style="display: flex; align-items: center; gap: 18px;">
    <img src="https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=120&h=120&fit=crop&crop=center" style="
      width: 52px; height: 52px; border-radius: 14px;
      object-fit: cover; box-shadow: 0 8px 24px rgba(91,106,245,0.4);
      flex-shrink: 0; border: 2px solid rgba(91,106,245,0.5);
    " alt="Ritesh Funds Advisor">
    <div>
      <div style="
        font-family: 'Syne', sans-serif;
        font-size: 26px;
...
with st.sidebar:
    ...
    <div style="font-size:11px;color:#6b7194;margin-bottom:12px;line-height:1.5;">
      ...
      Each slider is capped so totals never exceed 100%.
    </div>
    """, unsafe_allow_html=True)

    alloc_pcts = {}
    # We'll collect sliders sequentially; for each one, cap max at remaining budget
    used_so_far = 0
    raw_vals = {}
    last_idx = len(DEFAULT_ALLOC) - 1
    for i, a in enumerate(DEFAULT_ALLOC):
        # Last slider is always auto-calculated — no slider rendered
        if i == last_idx:
            break
    ...
    # Last asset always gets the remainder to enforce exactly 100%
    ...
    <div style="margin-top:14px; padding:12px 14px; border-radius:10px;
      ...
      <div style="font-family:'Syne',sans-serif;font-size:22px;font-weight:800;color:#10b981;">100%</div>
      <div style="font-size:11px;color:#10b981;margin-top:2px;">✓ Always perfectly balanced</div>
    </div>
    """, unsafe_allow_html=True)
    st.markdown(f"""
    <div style="margin-top:8px;font-size:11px;color:#6b7194;">
      {" &nbsp;·&nbsp; ".join([f"<b style='color:#dde1f0'>{a['label']}</b> {alloc_pcts[a['key']]}%" for a in DEFAULT_ALLOC])}
    </div>
    """, unsafe_allow_html=True)
...
with c1:
    salary = st.number_input("Monthly take-home salary (₹)", min_value=0, step=1000, value=0, format="%d", help="Enter your net monthly salary after deductions — e.g. ₹75,000")
with c2:
    invest_raw = st.number_input("Amount you can invest (₹/mo)", min_value=0, step=500, value=25000, format="%d", help="Your fixed monthly investment amount")
with c3:
...
with hs_col2:
    checks = [
        ("Savings rate ≥ 20%", pct_of_sal >= 20, f"Currently {pct_of_sal:.1f}%"),
        ("Allocation sums to 100%", 95 <= total_pct <= 105, f"Currently {total_pct}%"),
        ("Emergency fund in place", has_emergency, "Set in sidebar"),
        ("Long-term horizon (7+ yrs)", proj_years >= 7, f"Currently {proj_years} yrs"),
        ("Diversified across asset classes", len([v for v in alloc_pcts.values() if v > 0]) >= 4, f"{len([v for v in alloc_pcts.values() if v > 0])} active classes"),
...
with tab2:
    ...
    # Can we achieve with current invest?
    ...
    if not has_emergency:
        ...
        ef_sip_needed = ef_target / 12  # save in 1 year
        st.markdown(f"""
...
with tab5:
    ...
    with pf1:
        filter_opt = st.radio("Asset class", ["All","Stocks","ETFs","Mutual Funds","FD / Debt","PPF/NPS","Gold"], horizontal=True, label_visibility="collapsed")
    with pf2:
    ...
    cat_map  = {"All":None,"Stocks":"shares","ETFs":"etf","Mutual Funds":"mf","FD / Debt":"fd","PPF/NPS":"ppf","Gold":"gold"}
